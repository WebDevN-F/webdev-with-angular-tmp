...
export class TourWizardService<T extends TourWizardStep = TourWizardStep> {
  ...
  private _hideStep(step: T | any): void {
  ...
  private _loadStep(stepId: number | string): T | any {
  ...
  private _showStep(step: T | any): void {
    const anchor = this.anchors[step && step.anchorId];
    if (!anchor) {
